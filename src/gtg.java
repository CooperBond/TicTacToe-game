...
public class gtg extends JFrame {
    ...
    public void handleHumanTurn(int number) {
        if (game .isCellFree(number)) {
            game.makeTurn(number, game.HUMAN);
            printGameTable();
            if (game.getWinner() == game.HUMAN) {
            ...
            printGameTable();
            if (game.getWinner() == game.COMPUTER) {
    ...
    public void printGameTable() {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                cells[i][j].setText(String.valueOf(game.gameTable[i][j]));
            }
        }
    }
